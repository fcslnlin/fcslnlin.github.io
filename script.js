...
if (heroParallax && heroImageWrap) {
  window.addEventListener('scroll', () => {
    ...
    heroParallax.style.transform = `translateY(${progress * 140}px)`;
  }, { passive: true });
}
...
function initSlideshows() {
  document.querySelectorAll('.work-card-slideshow').forEach(slideshow => {
    ...
    if (audioToggle) {
      const mutedIcon = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><line x1="23" y1="9" x2="17" y2="15"></line><line x1="17" y1="9" x2="23" y2="15"></line></svg>';
      const unmutedIcon = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><path d="M19.07 4.93a10 10 0 0 1 0 14.14M15.54 8.46a5 5 0 0 1 0 7.07"></path></svg>';
      audioToggle.addEventListener('click', (e) => {
        e.stopPropagation();
        muted = !muted;
        const activeSlide = slides[current];
        if (activeSlide.tagName === 'VIDEO') activeSlide.muted = muted;
        audioToggle.innerHTML = muted ? mutedIcon : unmutedIcon;
      });
    }
  });
}
